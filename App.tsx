...
const App: React.FC = () => {
  ...
  const handleTrade = async (action: TradeAction) => {
    ...
    if (isWin) {
      ...
      setStats(prev => {
        ...
        return {
          ...
          // HEARTS CAP LOGIC: Hearts stay same or refill if we implement that, but don't exceed 3
          // hearts: Math.min(3, prev.hearts + 1) // Optional: heal on win? No, usually not in roguelikes.
        };
      });
    } else {
      SoundService.playLoss();
    ...
    // FAST GAME OVER: Reduced delay from 1000 to 250ms for snappiness
    if (!isWin && stats.hearts <= 1) {
        setTimeout(() => setIsGameOver(true), 250);
    }
  };
  ...
  const handleRespawn = () => {
    SoundService.playUnlock();
    // HARD RESET HEARTS to 3
    setStats(prev => ({ ...prev, hearts: 3, balance: 10000 }));
    setIsGameOver(false);
    loadNewScenario();
  };

  // --- NEW PULSE LOADING SCREEN ---
  if (isLoadingAuth) {
      return (
        ...
        return (
          <div className="flex flex-col items-center justify-center w-full h-full max-w-md mx-auto relative px-4 pt-12">
            ...
            <ChartCard 
                ...
                skin={stats.equippedSkin} // PASS SKIN
            />

            {/* FLOATING RESULT SHEET (Mobile Safe) */}
            {showResult && (
